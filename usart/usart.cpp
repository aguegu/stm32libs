...
Usart::Usart(USART_TypeDef * usart, uint32_t rcc_apbx_periph,
	...
	uint16_t length = sizeof(uint16_t) * _buff_size;

	_tx_buff.index_write = 0;
	_tx_buff.index_read = 0;
	_tx_buff.buffer = (uint16_t *) malloc(length);

	_rx_buff.index_write = 0;
	_rx_buff.index_read = 0;
	_rx_buff.buffer = (uint16_t *) malloc(length);

	memset(_rx_buff.buffer, 0, length);
	memset(_tx_buff.buffer, 0, length);
}
