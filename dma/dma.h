...
public:
	...
	void init(uint32_t peripheral_base_addr = 0, uint32_t memory_base_addr = 0,
			...
			uint32_t buffer_size = 0, uint32_t peripheral_increment =
					DMA_PeripheralInc_Disable, uint32_t memory_increment =
					DMA_MemoryInc_Disable, uint32_t peripheral_data_size =
					DMA_PeripheralDataSize_Byte, uint32_t memory_data_size =
					DMA_MemoryDataSize_Byte, uint32_t mode = DMA_Mode_Normal,
			uint32_t priority = DMA_Priority_Low, uint32_t m2m =
					DMA_M2M_Disable );
	...
	void setTransfer(uint32_t length, uint32_t direction = DMA_DIR_PeripheralDST);
